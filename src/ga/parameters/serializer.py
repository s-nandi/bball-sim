...
def _write_json(file_path: Path, obj):
    tqdm.write(f"creating @ {file_path}")
...
class ParametersSerializer:
    ...
    def serialize_parameters(
        ...
        if should_serialize:
            ...
            _write_json(path, [asdict(parameters) for parameters in parameters_list])
...
class ParametersDeserializer:
    ...
    def deserialize_parameters(self, generation_number: int) -> Sequence[Parameters]:
        ...
        path = self._basepath.joinpath(file_name)
        parameters_objs = _read_json(path)
        parameters_list = []
